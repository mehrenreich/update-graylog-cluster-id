...
# Usage example:
#
# $ ./update-graylog-cluster-id.py --host 192.168.1.2 --cluster-id ED596D72-1301-4C73-816E-B3813704DFBX
#
...
def get_cluster_id(collection):
    return collection.find_one({"type": "org.graylog2.plugin.cluster.ClusterId"})['payload']['cluster_id']


def update_cluster_id(collection, cluster_id):
    print(f'Setting Graylog Cluster ID to {cluster_id}')
    result = collection.find_one_and_update({"type": "org.graylog2.plugin.cluster.ClusterId"},
                                            {"$set": {
                                                "payload": {
